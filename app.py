...
# ===============================================================
# 13 --- VISUALIZADOR IFC EXPORTADO -----------------------------
# ===============================================================

import streamlit.components.v1 as components

st.markdown("## 🧱 Visor IFC con huella de carbono exportada")

if "ifc_exportado" in st.session_state:
    with open(st.session_state["ifc_exportado"], "rb") as f:
        contenido_ifc = f.read()

    import base64
    b64_ifc = base64.b64encode(contenido_ifc).decode()
    ...
    <head>
      <script type="module">
        import {{ IfcViewerAPI }} from 'https://cdn.jsdelivr.net/npm/web-ifc-viewer@1.0.144/dist/ifc-viewer-api.js';
        ...
        window.onload = () => {{
          ...
          viewer.grid.setGrid();
          viewer.axes.setAxes();
          viewer.IFC.setWasmPath("https://cdn.jsdelivr.net/npm/web-ifc@0.0.46/");

          // Decodificar base64 y convertir a Blob
          const b64Data = "{b64_ifc}";
    ...
    st.info("🔄 Exporta primero el IFC para visualizarlo aquí.")
